...
public class ConteudoDAO {
    ...
    public ArrayList<Conteudo> getConteudosAll() {
        ...
        while (c.moveToNext()) {
            conteudos.add(new Conteudo(c.getString(c.getColumnIndex("cod_conteudo")), c.getString(c.getColumnIndex("nome_conteudo")), c.getString(c.getColumnIndex("descricao_conteudo"))));
        }
        close();
        return conteudos;
    }
    ...
    public ArrayList<Conteudo> getConteudosDisc(String nomeDisc) {
        ArrayList<Conteudo> conteudos = new ArrayList<Conteudo>();
        open();
        c = db.rawQuery("SELECT * FROM Conteudo c, Disciplina_Conteudo dc where dc.cod_disciplina = (SELECT cod_disciplina FROM Disciplina WHERE " + " nome_disciplina = '" + nomeDisc + "') and c.cod_conteudo = dc.cod_conteudo;", null);
        while (c.moveToNext()) {
            conteudos.add(new Conteudo(c.getString(c.getColumnIndex("cod_conteudo")), c.getString(c.getColumnIndex("nome_conteudo")), c.getString(c.getColumnIndex("descricao_conteudo"))));
        }
        close();
